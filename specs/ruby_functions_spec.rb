/Test suite for ruby_functions_practice - contains at least one test per function contained in the ruby_functions_practice.rb library/
...
class Functions_Practice < MiniTest::Test
  ...
  def test_volume_of_sphere()
    sphere_volume = volume_of_sphere(10)
    assert_equal(31400000000.0,sphere_volume)
  end
  ...
  def test_days_until_christmas()
      /test runs with two defined dates in different years/
      days_to_xmas=days_until_christmas(Date.parse('20th Oct 2016'))
...
/test runs with two defined dates of birth/
    age=age_of_person(Date.parse('4th Jan 1966'))
    assert_equal(50,age)
